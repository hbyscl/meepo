				 ...
				 for (String taskName : children) {
					 if (isOwner(taskName, currentUuid)) {
						 ...
						 if (null != data) {
							 ...
							 ScheduleManager.scheduleTask(taskDefine, new Date(getSystemTime()));
						 }
					 }
				 }
			 }
			 ScheduleManager.clearLocalTask(ownerTask);
		}
		return false;
	}
